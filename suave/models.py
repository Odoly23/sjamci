...
from custom.models import BaseModel, Status, Municipality, AdministrativePost,\
                          Village, Category_Emp, TIpu_Programa, Sector, Faze, Year
...
class EkipaMember(BaseModel):
    ...
    benefisiariu = models.ForeignKey(
        ...
        related_name="team_members", verbose_name="Benefisiariu"
    )
...
class MainCustomer(BaseModel):
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new and not self.hashed:
            self.hashed = hashlib.blake2b(str(self.id).encode()).hexdigest()
            # PERBAIKAN: sebelumnya salah pakai MarketAssessment di sini
            MainCustomer.objects.filter(pk=self.pk).update(hashed=self.hashed)
...
class Competitor(BaseModel):
    ...
    def __str__(self):
        # PERBAIKAN: sebelumnya '{self.business} - {self.frequency}', kurang informatif
...
class FinancialAssessment(BaseModel):
    ...
    inventory_frequency  = models.CharField(max_length=100, blank=True, null=True, verbose_name="Frekuensia Monitorizasaun")
    ...
    annual_revenue       = models.DecimalField(max_digits=12, decimal_places=2, blank=True, null=True, verbose_name="Rendimento Kada Tinan ($)")
    ...
    def __str__(self):
        # PERBAIKAN: model asli pakai '{self.name}' yang tidak ada di model ini
        return f"{self.business} - {self.annual_revenue}"
...
class FixedAsset(BaseModel):
    financial = models.ForeignKey(
        FinancialAssessment, on_delete=models.CASCADE,
        related_name="assets", verbose_name="Avaliasaun Finanseiru"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new and not self.hashed:
            self.hashed = hashlib.blake2b(str(self.id).encode()).hexdigest()
            # PERBAIKAN: model asli salah pakai Competitor di sini
            FixedAsset.objects.filter(pk=self.pk).update(hashed=self.hashed)
...
class CreditInfo(BaseModel):
    ...
    repayment_status = models.CharField(max_length=10, choices=REPAYMENT_CHOICES, blank=True, null=True, verbose_name="Situasaun Pagamentu Kreditu")
    ...
    collateral_amount = models.DecimalField(max_digits=12, decimal_places=2, blank=True, null=True, validators=[MinValueValidator(0)], verbose_name="Montante Kolateral / Garantia ($)")
    approved_by_bank = models.CharField(max_length=200, blank=True, null=True, verbose_name="Aprovasaun Husi Banku (ex: BNCTL)")
    ...
    def __str__(self):
        return f"{self.business} - {self.provider or 'Laiha provider'}"
